...
namespace AppBookingTour.Infrastructure.Data.Repositories
{
    public class StatisticsRepository : IStatisticsRepository
    {
        ...
        public async Task<(IEnumerable<ItemStatisticDTO> Items, int TotalCount, int TotalPages)> GetItemRevenueStatisticsAsync(
            DateOnly startDate,
            DateOnly endDate,
            ItemType itemType,
            int? pageIndex = null,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            parameters.Add("startDate", startDate.ToDateTime(TimeOnly.MinValue));
            parameters.Add("endDate", endDate.ToDateTime(TimeOnly.MaxValue));
            parameters.Add("bookingType", (int)itemType);
            parameters.Add("sttSuccess", _successStatuses);
            parameters.Add("sttAll", _allRelevantStatuses);
            ...
            switch (itemType)
            {
                ...
                default:
                    return (Enumerable.Empty<ItemStatisticDTO>(), 0, 0);
            }

            string sortSql = isDesc ? " ORDER BY TotalRevenue DESC" : " ORDER BY TotalRevenue ASC";
            ...
            if (pageIndex.HasValue && pageSize.HasValue && pageIndex.Value > 0 && pageSize.Value > 0)
            {
                ...
                parameters.Add("Take", pageSize.Value);
                pagingSql = " OFFSET @Skip ROWS FETCH NEXT @Take ROWS ONLY";
            }

            string finalSql = baseSql + sortSql + pagingSql;

            var rawResults = await _context.Database.GetDbConnection()
                .QueryAsync<RawRevenueStatisticResult>(finalSql, parameters);

            var items = rawResults.Select(r => new ItemStatisticDTO
            {
                ItemId = r.ItemId,
                ItemCode = r.ItemCode,
                ItemName = r.ItemName,
            ...
            int totalCount = rawResults.FirstOrDefault()?.TotalCount ?? 0;
        ...
        private class RawRevenueStatisticResult
        {
        ...
        public async Task<(IEnumerable<ItemStatisticByBookingCountDTO> Items, int TotalCount, int TotalPages)> GetItemBookingCountStatisticsAsync(
            DateOnly startDate,
            DateOnly endDate,
            ItemType itemType,
            int? pageIndex = null,
            ...
            CancellationToken cancellationToken = default)
        {
            var parameters = new DynamicParameters();
            parameters.Add("startDate", startDate.ToDateTime(TimeOnly.MinValue));
            parameters.Add("endDate", endDate.ToDateTime(TimeOnly.MinValue));
            parameters.Add("bookingType", (int)itemType);
            parameters.Add("sttSuccess", _successStatuses);
            parameters.Add("sttCanceled", _canceledStatuses);
            parameters.Add("sttAll", _allRelevantStatuses);
            ...
            switch (itemType)
            {
                ...
                default:
                    return (Enumerable.Empty<ItemStatisticByBookingCountDTO>(), 0, 0);
            }

            string sortSql = isDesc ? " ORDER BY totalCompletedBookings DESC" : " ORDER BY totalCompletedBookings ASC";
            ...
            var rawResults = await _context.Database.GetDbConnection()
                .QueryAsync<RawBookingCountStatisticResult>(finalSql, parameters);

            var items = rawResults.Select(r => new ItemStatisticByBookingCountDTO
            {
                ItemId = r.ItemId,
                ItemCode = r.ItemCode,
                ItemName = r.ItemName,
                ...
                ItemOptionCount = r.ItemOptionCount,
                rating = r.rating,
                cancellationRate = (r.TotalBookingsCount > 0)
                                    ? Math.Round((decimal)r.totalCanceledBookings / r.TotalBookingsCount, 4)
                                    : 0,
            })
            .ToList();
        ...
        private class RawBookingCountStatisticResult
        {
            public int TotalCount { get; set; }
            public int ItemId { get; set; }
            public string ItemCode { get; set; }
